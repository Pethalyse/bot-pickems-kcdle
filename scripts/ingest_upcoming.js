...
const { PANDASCORE_TOKEN, LEAGUE_ID_LEC, LEAGUE_ID_LFL, LEAGUE_ID_WORLDS, DATABASE_URL } = process.env;
const LEAGUE_IDS = [LEAGUE_ID_LEC, LEAGUE_ID_LFL, LEAGUE_ID_WORLDS].filter(Boolean);
if (!PANDASCORE_TOKEN || LEAGUE_IDS.length === 0) {
    throw new Error('Missing PANDASCORE_TOKEN or LEAGUE_ID_*');
}

const pool = new pg.Pool({ connectionString: DATABASE_URL });

function pick(o, k, d=null){ return o?.[k] ?? d; }
...
async function fetchUpcoming(ids) {
    const url = new URL('https://api.pandascore.co/lol/matches/upcoming');
    url.searchParams.set('per_page', '100');
    url.searchParams.set('filter[league_id]', ids.join(','));
    const res = await fetch(url, { headers: { Authorization: `Bearer ${PANDASCORE_TOKEN}` }});
...
async function main() {
    ...
    try {
        const upcoming = await fetchUpcoming(LEAGUE_IDS);
        await client.query('BEGIN');
        for (const m of upcoming) {
            const mapped = mapEntities(m);

            // upsert league/series/tournament
        ...
        console.log(`✅ Ingest OK: ${upcoming.length} matchs`);
    } catch (e) {
